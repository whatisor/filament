...
class_<AssetLoader>("gltfio$AssetLoader")
    ...
    /// createAssetFromGlb ::static method::
    /// buffer ::argument:: asset string, or Uint8Array, or [Buffer]
    /// ::retval:: an instance of [FilamentAsset]
    .function("_createAssetFromGlb", EMBIND_LAMBDA(FilamentAsset*,
            (AssetLoader* self, BufferDescriptor buffer), {
...
class_<ResourceLoader>("gltfio$ResourceLoader")
    .constructor(EMBIND_LAMBDA(ResourceLoader*, (Engine* engine), {
        return new ResourceLoader({ engine, utils::Path("http://"), true, true });
    }), allow_raw_pointers())
    ...
    .function("loadResources", EMBIND_LAMBDA(void, (ResourceLoader* self, FilamentAsset* asset), {
        // TODO: bind directly, bypass the lambda?
        self->loadResources(asset);
    }), allow_raw_pointers());
