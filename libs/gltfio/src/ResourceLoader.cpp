...
bool ResourceLoader::createTextures(details::FFilamentAsset* asset) const {
    ...
    tsl::robin_map<const void*, Texture*> binTextures;
    tsl::robin_map<std::string, Texture*> strTextures;
    ...
    for (size_t i = 0, n = asset->getTextureBindingCount(); i < n; ++i) {
        ...
        if (tb.data) {
            ...
            if (!tex) {
                ...
                binTextures[data8] = tex = createTexture(texels, width, height, tb.srgb);
            }
        } else {
            tex = strTextures[tb.uri];
            if (!tex) {
                ...
                strTextures[tb.uri] = tex = createTexture(texels, width, height, tb.srgb);
            }
        }
